...
//## inner function, should use `paddr_read(paddr_t addr, int len)` instead
...
/* check paddr is valid and return stored value.
...
word_t paddr_read(paddr_t addr, int len) { 
  ...
  IFDEF(CONFIG_DEVICE, return mmio_read(addr, len)); //NOTE: access device IO memory access. mmio_read())->map_read()->host_read();
  out_of_bound(addr);
  return 0;
}
